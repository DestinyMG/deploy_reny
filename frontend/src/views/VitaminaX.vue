...
const factorVitamina = () => {
    if (!vitamina.value) return 1
    const vit = parseFloat(vitamina.value) || 0
    return (100 - vit) / 100
}
...
const calcularX = () => {
    ...
    if (calculando.value) {
        console.warn('Cálculo prevenido - posible bucle')
        return
    }
    ...
    try {
        ...
        const vit = parseFloat(vitamina.value)

        if (isNaN(obj) || isNaN(vit) || datos.some(v => isNaN(v))) {
            error.value = "⚠️ Completa todos los campos."; return
        }

        const datosOriginales = [...datos]
        const menores = datosOriginales.filter(v => v < obj)
        const mayores = datosOriginales.filter(v => v > obj)
        ...
        for (let i = 0; i < mitad; i++) {
            x_final[i] = (datosOriginales[n - 1 - i] - obj) / K
            x_final[n - 1 - i] = (obj - datosOriginales[i]) / K
        }

        const sumaX = x_final.reduce((a, b) => a + b, 0)
        x_final = x_final.map(x => x / sumaX)

        const factorDecimal = factorVitamina()
        const proporcionesAjustadas = x_final.map(x => x * factorDecimal * 100)

        resultados.value = {
            detalles: ingredientes.value.map((ing, idx) => ({
                index: idx + 1,
                proporcionX: proporcionesAjustadas[idx],
                x_decimal: x_final[idx],
                valorOriginal: ing.valor,
                aporteProteico: (ing.valor * (proporcionesAjustadas[idx] / 100))
            })),
            sumaX: factorDecimal * 100,
            factorVitamina: factorDecimal * 100,
            vitaminaPorcentaje: vit,
            aporteTotal: obj,
...
const actualizarNutrientes = () => {
    ...
    try {
        ...
        ingredientes.value.forEach((ing, idx) => {
            const prop = resultados.value.detalles[idx].proporcionX / 100
...
// --- WATCH PARA VITAMINA (CORREGIDO) ---
watch(vitamina, (nuevoValor, valorAnterior) => {
    // Solo recalcular si el valor realmente cambió y hay resultados
    if (resultados.value && nuevoValor !== valorAnterior && !calculando.value) {
        calcularX()
    }
})
        ...
        <div class="max-w-[1300px] mx-auto px-2 sm:px-6 py-6 space-y-6">
            ...
            <div class="flex flex-col gap-4 border-b border-white/10 pb-6">
                <div class="flex justify-between items-center">
                    <h2 class="text-2xl font-black text-white italic tracking-tighter uppercase">VITAMINA <span
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div
                        class="bg-[#020617] p-4 rounded-2xl border border-emerald-500/30 flex justify-between items-center shadow-xl">
                        <span class="text-[10px] font-black text-emerald-400 uppercase leading-none">Vitamina<br>a
                            Restar:</span>
                        <div class="flex items-center gap-2">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-center min-w-[500px]">
                                <thead
                                    ...
                                    <tr>
                                        <th class="p-4">ID</th>
                                        <th class="p-4">Proteína</th>
                                        <th class="p-4 text-indigo-400 italic">Inclusión final</th>
                                        <th class="p-4 text-right pr-6">Aporte proteico</th>
                                    </tr>
                                </thead>
                                ...
                                <tfoot class="bg-indigo-600/10 border-t border-indigo-500/30">
                                    ...
                                    <tr class="font-black italic text-sm bg-indigo-600/20">
                                        <!-- <td colspan="3"
                                            ...
                                            Vitamina restada</td>
                                        <td class="p-4 text-right pr-6 text-emerald-400 text-xl">{{ vitamina }}%</td> -->
                                    </tr>
                                    <tr class="font-black italic text-sm bg-indigo-600/20">
                                        <!-- <td colspan="3"
                                            class="p-4 text-[9px] text-indigo-400 uppercase tracking-tighter text-left">
                                            TOTAL + VITAMINA</td>
                                        <td class="p-4 text-right pr-6 text-white text-xl">{{ (resultados.sumaX +
                                            vitamina).toFixed(4) }}%</td> -->
                                    </tr>
                                    <tr class="font-black italic text-sm bg-indigo-600/20">
                                        <td colspan="3"
                                            class="p-4 text-[9px] text-indigo-400 uppercase tracking-tighter text-left">
                                            Aporte Proteico Total</td>
                                        <td class="p-4 text-right pr-6 text-white text-xl">{{
